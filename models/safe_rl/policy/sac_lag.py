"""Soft Actor-Critic with PID-Lagrangian safety constraints (SAC-Lagrangian).

Extends the maximum-entropy SAC algorithm [1] with multiple safety
constraints enforced via PID-controlled Lagrange multipliers [2].  Three
soil-moisture constraints are supported:
...
References
----------
[1] T. Haarnoja, A. Zhou, P. Abbeel, and S. Levine, "Soft Actor-Critic:
    Off-Policy Maximum Entropy Deep Reinforcement Learning with a
    Stochastic Actor," in Proc. 35th Int. Conf. Mach. Learn. (ICML),
    PMLR 80, 2018.
...
from models.safe_rl.policy import SAC, LagrangianPIDController
...
class SACLagrangian(SAC):
    """SAC with PID-Lagrangian safety constraints [1, 2].

    The actor loss augments the standard entropy-regularised objective with
    Lagrangian penalty terms -- one per constraint -- whose multipliers are
    adapted online by PID controllers following Stooke et al. (2020) [2].
    ...
    **kwargs
        Forwarded to :class:`SAC` (gamma, hidden_sizes, lr, etc.).
    """
    ...
    def _update_actor(self, data: dict) -> None:
        ...
        The loss combines the entropy-regularised SAC objective with
        Lagrangian penalty terms for each safety constraint.  PID
        ...
        def policy_loss():
            ...
            # --- Policy loss ---
            current_alpha = (
                self.log_alpha.exp().item() if self.learn_alpha else self.alpha
            )
            ...
            # --- Entropy temperature loss (SAC dual variable) ---
            alpha_loss = None
            if self.learn_alpha:
                alpha_loss = -(
    ...
    def _update_qc(self, data, qc, qc_targ, qc_optimizer, key: str) -> None:
        ...
        * **Standard** (SW): additive cost backup (cost = 1 - is_safe) with MSE.
        ...
        def critic_loss():
            ...
            with torch.no_grad():
                act_next, _ = self.actor_forward(
                    obs_next, deterministic=False, with_logprob=False
                )
    ...
    def post_epoch_process(self) -> None:
        """End-of-epoch housekeeping: alpha, thresholds, regularization."""
        self._update_alpha()
