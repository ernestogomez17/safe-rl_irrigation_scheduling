...
class WaterEnvironment(gym.Env):
    ...
    KCINI = 0.4    # Late growth stage length
    KCMID = 0.85    # Late growth stage length
    KCEND = 0.35    # Late growth stage length
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.training_mode:
            self.seed = int(np.random.uniform(self.S_STAR, self.SFC))  # Random seed
        else:
            self.seed = seed if seed is not None else 42  # Fixed or fallback evaluation seed

        np.random.seed(self.seed)
        self.st = np.random.uniform(self.S_STAR, self.SFC)
        ...
        if self.training_mode:
            random_date = np.random.choice(['2015-04-10', '2016-04-10'])
        else:
    ...
    def advance_simulation(self):
        # print(f"Before increment: elapsed_days = {self.elapsed_days}")
        # print(f"n_days_ahead: ", self.n_days_ahead)
        if (self.elapsed_days + self.n_days_ahead) < self.total_days:
    ...
    def calculate_ET_o(self):
        ...
        Rnl = (4.903 * 10**-9) * (((self.current_Tmax + 273.16)**4 + (self.current_Tmin + 273.14)**4) / 2) * (0.34 - 0.14 * np.sqrt(ea)) * (1.35 * (Rs/Rso) - 0.35)
    ...
    def update_environment(self):
        ...
        if self.training_mode:
            # Add some noise to current_rho as before
            self.current_rho = self.current_rho * np.random.uniform(0.9, 1.1)

            if self.current_Rain == 0:
                # Use an exponential distribution to introduce noise, typically near zero but occasionally larger
                scale = np.random.choice([1, 1.5, 2])
                self.current_Rain = np.random.exponential(scale=scale)
                self.current_Rain = min(self.current_Rain, 15)  # Clip the noise to not exceed 5
            else:
                # Apply normal uniform noise when rain is non-zero
                self.current_Rain = self.current_Rain * np.random.uniform(0.60, 1.4)
    ...
    def _get_obs(self):
        # Cyclical representations for time-related features
        month = np.sin(2 * np.pi * self.month_of_the_year / 12)
        day = np.sin(2 * np.pi * self.day_of_the_year / 365)
        week = np.sin(2 * np.pi * self.week_of_the_year / 52)

        # Normalize other state variables based on their maximum possible values
        normalized_rho = self.current_rho
        normalized_rain = self.current_Rain / max(self.Rain_base)

        # Ensure values remain in a valid range using clipping if needed
        normalized_rho = np.clip(normalized_rho, 0, 1)
        normalized_rain = np.clip(normalized_rain, 0, 1)

        # Return the observation with normalized values
        obs = np.array([month, day, week, self.st, normalized_rho, normalized_rain], dtype=np.float32)
    ...
    def step(self, action):
        ...
        # Loop until the specified number of days ahead
        while self.n_day_counter < self.n_days_ahead:
            ...
            #print(f"Step {self.n_day_counter}: Computed Costs: S_STAR={cost_s_star}, SFC={cost_sfc}, SW={cost_sw}")

            # total_cost_s_star += cost_s_star
            # total_cost_sfc += cost_sfc
            # total_cost_sw += cost_sw
            #print(f"Accumulated Costs: S_STAR={total_cost_s_star}, SFC={total_cost_sfc}, SW={total_cost_sw}")
        ...
        self.terminated = False
        self.truncated = (self.elapsed_days + self.n_days_ahead) >= self.total_days
